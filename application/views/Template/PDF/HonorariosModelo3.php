...
*/  $json						=	json_decode($empresa->json);
    ...
    $registro                   =   get_registro_contable_honorarios_new($this->uri->segment(3),$this->uri->segment(4));
     /* echo '$json';
      pre($json);
      echo '$ciclo_informacion ';
      pre($ciclo_informacion );
      echo '$escala_escala_x_user_id';
      pre($escala_escala_x_user_id);
      echo "$ciclos_pagos_end";
...
?>
                        <?php
                            $data_fecha = calculo_fechas($ciclo_informacion->fecha_hasta,$cantidad_dias='+1');
                            if($data_fecha > date("Y-m-d")){
                                $trm_now                                =   trm_vigente(true);
                            }else{
                                $trm_now                                =   periodotrm($data_fecha)->monto;
                            } 
                        ...
                        ?>
                                ...
                                ?>
                                            <?php 
                                                ...
                                                $pro = $totalP         =   $totalP + @$items_factura_x_nickname->monto_global_usd / 0.05;
                                ...
                                ?>
                                                    <?php 
                                                        if(isset($v->recurrente) && $v->recurrente==1){
                                ...
                                ?>
                                <?php 
                                    $bonificacion       =   calcular_bonificacion($escala_escala_x_user_id->meta,$totalP,$factorBonificacion,$trm_now);
                    ...
                    <?php }?>
                                ...
                                ?>
                                        ...
                                        <?php $EscalaP = $escala->prima; $rp_honorarios_modelos["escala_prima_porcentaje"]=$escala->prima;?>
                                            ...
                                            <?php $beneficiosSinPunto = $total_beneficios; $totalBeneficios = format($total_beneficios,false); $rp_honorarios_modelos["total_beneficios"]=$total_beneficios;?>
                                            <?php 
                                                ...
                                                $totalIngresosSinPunto = $total_liquidacion_ingresos;
...
*/
    ...
    $monitor = centrodecostos($row->monitor);
    $num=0;
    ...
    <tr>
        <td width="50%" colspan="2">
            ...
            <?php
                if($Logo_Empresa == 1){
            ?>
            <img src="<?php echo img_logo($empresa->Logo_Corporativo->user_id);?>" style="width:153px;height:90px; margin-left: 10px;" />
            <?php
                }
            ?>
        </td>
...
<div style="font-family:font-family: 'Montserrat', sans-serif; line-height:16px; font-size:12px; font-family:font-family: 'Montserrat', sans-serif;">
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
        ...
        <tr>
            <td class="borderleft bordeBottom">
                <b><?php echo $row->primer_nombre.' '.$row->segundo_nombre.' '. $row->primer_apellido .' '.$row->segundo_apellido; ?></b><br>
                  Documento Identificación: <b><?php echo format($row->identificacion,false); ?></b><br>
                  Dirección: <b><?php echo $row->direccion; ?></b><br>
                  Ciudad: <b><?php echo $row->ciudad; ?></b><br>
                  Sucursal: <b><?php echo $Sucursal; ?> - (Room# <?php if($row->room_transmision != 1000000){echo $row->room_transmision;}else{echo 'Satelite';} ?>)</b> - Turno: <b><?php echo @$turno; ?></b><br>
                  Entidad Bancaria: <b><?php echo entidadbancaria($row->entidad_bancaria); ?></b><br>
                  Cuenta Bancaria: <b><?php echo $row->nro_cuenta; ?></b><br>
                  Factor bonificación: <b><?php echo $factorB; ?></b>
                  Tokens Bonificación:
                <b>
                    <?php
                        $TKBon  = $totalP - $escala_escala_x_user_id->meta;
                        if($TKBon<0){
                          $TKBon = 0;
                        }
            ...
            <td class="borderight bordeBottom">
                Ciclo de pago: 
                <b>
                    <?php 
                ...
                - Meta <b>(<?php echo format($escala_escala_x_user_id->meta,false); ?>)
                </b>
                <br>
                Días transmitidos:
                    <b>
            ...
            Clasificación: <b><?php echo @$row->tipo_modelo; ?> (<?php echo @$monitor->primer_nombre.' '.@$monitor->primer_apellido; ?>)</b><br>
                Valor TRM:
                    <b>
                        $ <?php 
                            echo @$Valor_TRM;
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:10px;">
        ...
        <tr>
            <td class="borderleft">Pacto comercial (<b>Auxilio E.P.S.</b>)</td>
        ...
        <tr>
            <td class="borderleft">Pacto comercial (<b>Auxilio A.R.L.</b>)</td>
            <td class="borderight" style="text-align: right;">
              <b>
                <?php
                    echo '$ '.$AuxARL;
        ...
        <tr>
            <td class="borderleft" width="160">Pacto comercial (<b>Auxilio Caja C. Fliar</b>)</td>
            ...
            <td width="160">Pacto comercial (<b>Bonificación quincenal</b>)</td>
            <td class="borderight" style="text-align: right;" width="61">
                <b>
                    <?php 
                        echo '$ '.$Bon;
                    ?>
        ...
        <tr>
            <td colspan="2" class="borderleft" width="300">Escala de pago SANCIONADA y RETIRADA será, liquidada sobre el 50%</td>
            <td class="colorFondo" width="180">Total ingresos percibidos + beneficios</td>
            <td class="colorFondo" style="text-align: right;padding-right: 4px"><?php $totalBeneficiosIngresos = $beneficiosSinPunto + $totalIngresosSinPunto; echo '$ '.format($totalBeneficiosIngresos,false) ?></td>
        </tr>
        <tr>
            <td colspan="4" class="bordeBottom borderight borderleft" style="text-align: justify;">*<b>Importante: <?php echo $empresa->nombre_legal?></b> sólo pagára los valores denominados "PROVISIÓN AUXILIO SEMESTRAL", cuando no exista terminación de contrato o retiro de la modelo, antes del día 20 de Junio y/o Diciembre respectivamente.</td>
        </tr>
    </table>
    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:10px;">
        <thead>
            <tr class="colorFondo">
                <td colspan="4">Detalle de producción por plataforma.</td>
            </tr>
            <tr class="colorFondo">
                <th><b>Plataforma</b></th>
                <th><b>Usuario</b></th>
                <th class="text-right"><b>Producción</b></th>
                <th class="text-right"><b>Discriminado en pesos</b></th>
            </tr>
        </thead>
        <tbody>
            <?php 
                $totalsumaproduccion = 0;
                $totalRQ        =   0;
                $totalP         =   0;
                $rp_honorarios_modelos["HonorariosModelos"]             =   $HonorariosModelos  =   $this->$modulo->HonorariosModelos($this->$modulo->result->user_id);
                foreach($rp_honorarios_modelos["HonorariosModelos"] as $k => $v){
            ...
            ?>
                <tr>
                    ...
                    <td class="borderleft borderight" style="text-align: right; padding-right: 5px;">
                        <?php
                            if($totalBeneficiosIngresos != 0 && $pro != 0 && $conversion != 0){
                                $totalsumaproduccion += ($totalBeneficiosIngresos/$pro)*$conversion;
                                $dis_pesos = '$ '.format(($totalBeneficiosIngresos/$pro)*$conversion,false);
                            }else{
                                $dis_pesos = '$ 0';
                            }
                            echo $dis_pesos;
        ...
        <tfoot>
            <tr>
                ...
                <td class="colorFondo"><b>Total producción</b></td>
                ...
                <td class="text-right colorFondo" style="text-align: right;">
                    <b> 
                        <?php echo '$ '.format($totalsumaproduccion,false);?>
                    </b>
                </td>
            </tr>
        </tfoot>
    ...
    <table  border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:10px;">
        ...
        <tbody>
            ...
            ?>
            <tr>
                <td class="borderleft">N.A.</td>
                <td class="borderleft">N.A.</td>
                <td class="borderleft">N.A</td>
                <td class="borderleft" style="text-align: right;">$ 0</td>
                <td class="borderleft borderight" style="text-align: right;">$ 0</td>
            </tr>
            <?php
                }
            ?>
        ...
        <tfoot>
            <tr>
                <td colspan="3" class="bordeAll" style="text-align: justify;">Dineros recibidos para terceros, dudas favor consultar al tercero que reporta el descuento.</td>
                <td class="text-center colorFondo"><b>Total Descuentos</b></td>
                <td class="text-right colorFondo" style="text-align: right;"><b>$ <?php echo $Descu; ?></b></td>
            </tr>
        </tfoot>
    </table>
    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:10px;">
        ...
        <tbody>
                    ...
                    <b>
                        <?php
                            if(!empty($metaIdeal)){
                                ...
                                echo $escala_escala_x_user_id->meta;
                            }
                        ?>
                    </b>
                </td>
                <td>Retención en la fuente (
                    <?php
                        if(empty($config->porcentaje_retencion)){
                            ...
                            $PorcentajeRete = $rp_honorarios_modelos["porcentaje_retencion"]   =    format($config->porcentaje_retencion,true); 
                        }
                        echo $porcentajeRete;
                    ?> %)</td>
                <td class="borderight" style="text-align: right;">
                    <b>
                        <?php
                            $prueba =  @$config->porcentaje_retencion * $total_liquidacion_ingresos /100;
                            echo '$ '.format($prueba,false);
                        ?>
                    </b>
                </td>
            </tr>
            <tr>
                ...
                <td class="borderight bordetop" style="text-align: right;">
                    <b>
                        <?php
                            if(empty($metaIdeal)){
                                if(empty($escala_escala_x_user_id->meta)){
                                    $escala_escala_x_user_id->meta = 7560;
                                }
                                $cumplimiento = ($rp_honorarios_modelos["totalP"]*100)/$escala_escala_x_user_id->meta;   
                            }else{
                                $cumplimiento = ($rp_honorarios_modelos["totalP"]*100)/@$metaIdeal;
                            }
                            echo format($cumplimiento,false);
                        ?> %
                    </b>
                </td>
            ...
            <tr>
                <td style="text-align: justify;font-size: 10px;" rowspan="3" colspan="2" class="bordeBottom borderleft bordetop">
                        Importante: Pérdida de beneficios del pacto comercial cuando: a) conexión o llegadas tarde más de dos (2) veces en el periodo, b) falta sin justa causa, c) NO atención a members, d) NO atención monitores, ante pérdida de beneficios la modelo asumirá costo de seguridad social de ese periodo, el porcentaje de pago será del 50%, fórmula (TRMX0,88X0,05X50%).
                </td>
                <td class="borderleft">Subtotal</td>
                <td style="text-align: right;" class="borderight">
                    <b>
                        <?php
                            $prueba2 = $total_liquidacion_ingresos - $prueba - $total_monto_cuota;
                            echo '$ '.format($prueba2,false);
                        ?>        
                    </b>
                </td>
            </tr>
            <tr>
                ...
                <td class="borderight" style="text-align: right;"><b><?php echo '$ '.format(ajuste_a_la_decena($prueba2) - $prueba2,false); ?></b></td>
            </tr>
            <tr>
                <td class="colorFondo"><b>Total liquidación de ingresos finales</b></td>
                <td style="text-align: right;" class="colorFondo"><b><?php echo '$ '.format(ajuste_a_la_decena($prueba2),false); ?></b></td>
            </tr>
        </tbody>
    </table>
</div>
