...
function Clientes($row,$name,$placeholder='',$require=false){
	...
	function ResumenBancos($incluir=array('130510'),$tipo_documento=array("Comprobante Bancario"),$tipo_monedas=array("Euros","USD","Pesos")){
		$ci 	=& 	get_instance();		
		$tabla	=	DB_PREFIJO."fi_cuentas t1";
		return $ci->db->select("sum(credito) - sum(debito) as total,consecutivo,codigo_contable,debito,credito,entidad_bancaria,nro_cuenta,fecha,procesador_id,tipo_cuenta,t1.id_cuenta")
									->from($tabla)
									...
									->get()
									->result();
		return $ci->db->select("sum(credito) as total,consecutivo,codigo_contable,debito,credito,entidad_bancaria,nro_cuenta,fecha,procesador_id,tipo_cuenta,t1.id_cuenta")
									->from($tabla)
									->join(DB_PREFIJO."rp_operaciones t2", 't2.procesador_id = t1.id_cuenta', 'left')
									->where('t2.id_empresa',$ci->user->id_empresa)
									->where_in("t2.codigo_contable",$incluir)
									->where_in("t1.tipo_monedas",$tipo_monedas)
									->where_in("t2.tipo_documento",$tipo_documento)
									->group_by('t1.nro_cuenta','ASC')
									->get()
									->result();									
	}
	
	function RetirosTRMDetalles($incluir=array('130510'),$tipo_documento=array("TRM Retiro")){
		$ci 	=& 	get_instance();		
	...
	function RetirosTRMDetalle($id){
		$ci 	=& 	get_instance();		
		$tabla	=	DB_PREFIJO."rp_retiroTRM t1";
		$trm	=	$ci->db->select("*")
										->from($tabla)
										->join(DB_PREFIJO."fi_cuentas t2", 't1.id_cuenta = t2.id_cuenta', 'left')
										->join(DB_PREFIJO."sys_bancos t3", 't1.banco_id = t3.banco_id', 'left')
										->where('t1.id_empresa',$ci->user->id_empresa)
										->where("t1.consecutivo",$id)
										->get()
										->row();

		$tabla				=	DB_PREFIJO."rp_operaciones t1";		
		$cuenta_contable	=	$ci->db->select("credito, debito,consecutivo,codigo_contable,debito,credito,fecha,procesador_id")
													->from($tabla)
													->where('t1.id_empresa',$ci->user->id_empresa)
													->where("t1.consecutivo",$trm->consecutivo)
													->get()
													->result();	
		return array("trm"=>$trm,"cuenta_contable"=>$cuenta_contable);																								
	}
	...
	function ResumenCajas($incluir=array('110505'),$tipo_documento=array("TRM Retiro")){
		...
		return $ci->db->select("IFNULL(SUM(credito), 0) as total,consecutivo,codigo_contable,debito,credito,fecha,procesador_id,nombre_caja")
									->from($tabla)
									->join(DB_PREFIJO."rp_operaciones t2", 't1.id_caja= t2.procesador_id', 'left')
									->where('t1.id_empresa',$ci->user->id_empresa)
									->where_in("t2.codigo_contable",$incluir)
									->where_in("t2.tipo_documento",$tipo_documento)
									->get()
		...
		/*
		$tabla	=	DB_PREFIJO."fi_cajas t1";
		return	$ci->db->select("nombre_caja,(SELECT SUM(credito) 
												FROM ".DB_PREFIJO."rp_operaciones
													WHERE  id_empresa='".$ci->user->id_empresa."'
														AND codigo_contable='".$ci->user->id_empresa."'
															AND id_empresa='".$ci->user->id_empresa."') as total")
									->from($tabla)
									->join(DB_PREFIJO."rp_operaciones t2", 't1.id_caja 	= 	t2.procesador_id', 'right')
									->where('t1.id_empresa',$ci->user->id_empresa)
									->get()
									->result();
									*/
		...
		$tabla	=	DB_PREFIJO."rp_operaciones t1";			
		foreach($rows as $k =>$v){
			$return[$v->id_caja] =	$ci->db->select("'".$v->nombre_caja."' AS nombre_caja, IFNULL(SUM(credito), 0) as total,IFNULL(SUM(credito_COP), 0) as total_COP,consecutivo,codigo_contable,debito,credito,fecha,procesador_id")
														->from($tabla)
														->where('t1.id_empresa',$ci->user->id_empresa)
														->where('t1.procesador_id',$v->id_caja)
        	...
        	$html	.=	'<div class="col-md-12">';
				...
				foreach($rows as $k =>$v){	
					...
					$html	.=	'			<tr>
												...
												<td class="text-center">'.$json->consecutivo.'</td>
												<td>'.nombre($v).'</td>
												<td class="text-right">'.$json->debito.'</td>';
					if($json->codigo_contable=='414580'){
						$html	.=	'				<td class="text-right">'.$json->usd.'</td>';	
					}else{
						$html	.=	'				<td class="text-right">'.$json->credito.'</td>';	
					}
					$html	.=	'				</tr>';
				}
				$html	.=	'		</tbody>
	...
	function bancos($row, $estado = null,$extra=array("class"=>"form-control"),$name='entidad_bancaria'){
		...
		$html	=	'';
		$html	.=	'	<input type="text" class="form-control" id="content_entidad_bancaria"  placeholder="Entidad Bancaria" maxlength="200" value="'.@$row->Entidad.'" require   />';		
	...
	function Autocpmlete_CuentasBancarias($row, $estado = null,$extra=array("class"=>"form-control"),$subfuncion='',$name='id_cuenta',$tipo_cuenta="Crédito prepagada"){
		...
		$ci 	=& 	get_instance(); 
		$tabla	=	DB_PREFIJO."fi_cuentas";
		$rows	=	$ci->db->select("CONCAT(entidad_bancaria,'(',nro_cuenta,')') as entidad_bancaria,id_cuenta")
							->from($tabla)
							->where('estado',"1")
