...
class Ventas_model extends CI_Model {
	...
	public function get_ve_escala_pagos(){
		...
		$this->fields		=		array("id_escala"=>"id_escala","nombre_escala"=>"Nombre","meta"=>"Meta","CASE WHEN estado=1 THEN 'Activo' ELSE 'Inactivo' END"=>"Estado"," CONCAT('".$edit_open."',id_escala,'".$edit_close."') AS edit"=>"Acción");
	...
	public function get_ve_FormasPagos(){
		...
		$tabla				=		DB_PREFIJO."ve_forma_pagos t1";
  		$html_group_open	=		'<div class="btn-group btn-group-sm" role="group" aria-label="Small button group">';
		$html_group_close	=		'</div>';
		$edit_open			=		$html_group_open.'<a class="btn btn-secondary lightbox" title="Editar Forma de Pago" data-type="iframe" href="'.base_url($this->uri->segment(1).'/Add_FormasPagos/');
		$edit_close			=		'"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
