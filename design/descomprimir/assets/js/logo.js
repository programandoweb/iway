...
var Cropper = function () {
  ...
  _createClass(Cropper, [{
    ...
    value: function _export(img) {
		this.rendernorecord();
     	img.setAttribute('src', this.imageCanvas.toDataURL());
    }
    ...
    key: 'rendernorecord',
    value: function rendernorecord() {
		this.crop.size	=	{"x":this.imageCanvas.width,"y":this.imageCanvas.width}	
		this.crop.pos	=	{"x":0,"y":0}	;	
...
function init(dimensions){
	try {
	  ...
	  if (uploader && editor) {
		...
		document.querySelector('.js-norecort').onclick = function (e) {
			return editor.norecort(img);
		};
	} catch (error) {
